...
function update_video_status($db, $video_id) {
  ...
  $stmt = $db->prepare("UPDATE wp_uploaded_video SET status='delete', uploaded_time=:update_time WHERE video_id=:video_id");
  $stmt->execute(array(':video_id' =>  $video_id, ':update_time' => $time));
}
